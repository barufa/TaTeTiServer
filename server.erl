...
isavailiable(Pid,Nombre)->
	R=lists:any(fun(Node)->{dir,Node}!{ishere,self(),Nombre},receive X -> X after ?TIMEOUT -> false end end,nodes()),
	Pid!R.
...
getuserlist(Pid)->
	Lw = lists:map(fun(Node)->spawn(Node,server,getuserlistaux,[self()]),receive {lu,X} -> X after ?TIMEOUT -> ordsets:new() end end,[node()|nodes()]),
